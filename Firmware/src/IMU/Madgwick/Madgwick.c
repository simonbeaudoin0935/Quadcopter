...
static void Madgwick_compute_IMU(float gx, float gy, float gz, float ax, float ay, float az);
...
void Madgwick_compute(float gx, float gy, float gz, float ax, float ay, float az, float mx, float my, float mz){
	float recipNorm;
		...
		if((mx == 0.0f) && (my == 0.0f) && (mz == 0.0f)) {
			Madgwick_compute_IMU(gx, gy, gz, ax, ay, az);
			return;
		}
...
void Madgwick_compute_IMU(float gx, float gy, float gz, float ax, float ay, float az){
	float recipNorm;
		float s0, s1, s2, s3;
		float qDot1, qDot2, qDot3, qDot4;
